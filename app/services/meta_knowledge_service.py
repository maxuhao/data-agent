...
本模块实现智能问数系统的知识库构建功能，将配置文件中定义的表、字段、指标等信息同步到各个存储系统中。

功能说明：
1. 读取 meta_config.yaml 配置文件
2. 将表信息和字段信息保存到 Meta MySQL 数据库
3. 为字段信息建立 Qdrant 向量索引（支持语义检索）
4. 为维度字段取值建立 Elasticsearch 全文索引（支持模糊匹配）
5. 将指标信息保存到 Meta MySQL 数据库
6. 为指标信息建立 Qdrant 向量索引

核心流程：
【步骤 1】加载配置文件
    - 使用 OmegaConf 读取并验证 YAML 配置
    - 解析表定义、字段定义、指标定义

【步骤 2】同步表信息（如果配置了 tables）
    2.1 保存到 Meta MySQL
        - 保存 TableInfo（表结构信息）
        - 保存 ColumnInfo（字段详细信息）
        - 从事务 DW 库查询字段类型和取值示例
    ...
    2.2 建立向量索引（Qdrant）
        ...
        - 存入 Qdrant column_info 集合
        
    2.3 建立全文索引（Elasticsearch）
        - 只对标记为 sync=True 的字段
        - 查询字段的所有取值（最多 100000 条）
        - 批量索引到 ES value_index

【步骤 3】同步指标信息（如果配置了 metrics）
    3.1 保存到 Meta MySQL
        - 保存 MetricInfo（指标定义）
        - 保存 ColumnMetric（指标与字段的关联关系）
        
    3.2 建立向量索引（Qdrant）
        - 为指标名生成向量
        - 为指标描述生成向量
        - 为每个别名生成向量
        - 存入 Qdrant metric_info 集合

技术栈：
- OmegaConf: YAML 配置加载和验证
- UUID: 生成唯一的向量 ID
- dataclasses.asdict: 实体对象 → 字典（用于序列化）
- 批量处理：分批次向量化，避免内存溢出
...
使用方法：
    ...
    await service.build(Path("conf/meta_config.yaml"))
